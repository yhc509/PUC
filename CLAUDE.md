...
- **QA ui-dump:** `qa ui-dump` dumps clickable UI elements with image-coordinate rect/center fields for direct reuse in `qa click --target` or `qa tap`; it requires Play Mode and has no force-rule.
