...
- **Auth token storage:** Live IPC auth tokens are stored in per-instance 0600 sidecars (`<registryDir>/tokens/<hash>.token`), not in the shared registry. The CLI reads the sidecar during resolve/load, and `InstanceRecord.token` is a non-serialized in-memory field. The instance registry entry and token sidecar are written before the listener is opened, survive domain reloads, and are removed only when the Editor exits. During a reload only the socket/listener is closed, so callers should see retryable `LIVE_UNAVAILABLE` until the bridge listener is back.
