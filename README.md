...
Assign a value to `__pucResult` when the caller needs a structured return value. The response includes `hasResult: true` and `result` as type-preserving JSON, with `float` formatted using G9 and `double` using G17 for round-trip precision. Custom `[PucCommand]` methods can return `ExecuteValueSerializer.Serialize(obj)` for the same precise JSON format.
