...
| `profile memory snapshot` | `profile memory snapshot` | live | `None` | Takes a full memory snapshot via MemoryProfiler.TakeSnapshot and saves it under Library/com.yhc509.unity-cli-bridge/snapshots/<id>.snap, returning only the path and metadata. Analyze the .snap in the Memory Profiler package GUI. |
